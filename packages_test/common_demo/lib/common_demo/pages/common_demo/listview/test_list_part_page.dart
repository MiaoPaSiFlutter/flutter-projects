...
class TestListPartPage extends StatefulWidget {
  const TestListPartPage({super.key});
...
class TestBean {
  ...
  TestBean({this.name, this.isCollect = false});
}
...
class TestListItemWidget extends StatefulWidget {
  ...
  const TestListItemWidget({required this.bean, Key? key}) : super(key: key);
...
class _ListItemState extends State<TestListItemWidget> {
  ...
  Row buildRow() {
    ...
    return Row(
      children: [
        ///权重布局 文本占用空白区域
        Expanded(child: Text("${widget.bean.name}")),
