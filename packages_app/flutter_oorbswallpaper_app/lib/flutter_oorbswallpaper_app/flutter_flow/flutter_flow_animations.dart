    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        ...
        if (animationInfo.curvedAnimation.status == AnimationStatus.dismissed) {
          if (animationInfo.hideBeforeAnimating != null &&
              animationInfo.hideBeforeAnimating!) {
            return Opacity(
              opacity: 0,
              child: child,
            );
        ...
        if (animationInfo.initialState?.offset.dx != 0 ||
          ...
          final xRange = (animationInfo.finalState?.offset.dx ?? 0.0) -
              (animationInfo.initialState?.offset.dx ?? 0.0);
          final yRange = (animationInfo.finalState?.offset.dy ?? 0) -
              (animationInfo.initialState?.offset.dy ?? 0);
          ...
          returnedWidget = Transform.translate(
            offset: Offset(
              (animationInfo.initialState?.offset.dx ?? 0) + xDelta,
              (animationInfo.initialState?.offset.dy ?? 0) + yDelta,
            ),
        ...
        if (animationInfo.initialState?.scale != 1 ||
          ...
          final range = (animationInfo.finalState?.scale ?? 0.0) -
              (animationInfo.initialState?.scale ?? 0.0);
          final delta = range * animationInfo.curvedAnimation.value;
          final scale = (animationInfo.initialState?.scale ?? 0.0) + delta;
        ...
        if (animationInfo.fadeIn) {
          final opacityRange = (animationInfo.finalState?.opacity ?? 0.0) -
              (animationInfo.initialState?.opacity ?? 0.0);
          final opacityDelta =
              animationInfo.curvedAnimation.value * opacityRange;
          final opacity =
              (animationInfo.initialState?.opacity ?? 0.0) + opacityDelta;
