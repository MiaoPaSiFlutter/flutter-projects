...
/*
高德地图、百度地图以及CLLocationManager等地图的定位功能，从而得到的经纬度坐标会有些偏差，
比如系统的CLLocationManager定位得到的是世界标准地理坐标(WGS-84)、
高德SDK定位得到的是火星坐标（GCJ-02）、
百度SDK定位得到的是百度地理坐标（BD-09），
想要实现经纬度坐标的统一整合，就必须实现经纬度之间的转换。
*/
...
class GpsUtil {
  ...
  static List<num> bd09_To_Gcj02(num lat, num lon) {
    ...
    num z = sqrt(x * x + y * y) - 0.00002 * sin(y * pi);
    num theta = atan2(y, x) - 0.000003 * cos(x * pi);
