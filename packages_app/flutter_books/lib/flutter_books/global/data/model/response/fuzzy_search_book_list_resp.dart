...
class FuzzySearchResp {
  ...
  factory FuzzySearchResp(jsonStr) => jsonStr is String
      ? FuzzySearchResp.fromJson(json.decode(jsonStr))
      : FuzzySearchResp.fromJson(jsonStr);
  ...
  FuzzySearchResp.fromJson(jsonRes) {
    ...
    for (var booksItem in books!) {
      books!.add(FuzzySearchList.fromJson(booksItem));
    }
  }
...
class FuzzySearchList {
  ...
  FuzzySearchList.fromParams(
      {this.wordCount, this.id, this.author, this.cat, this.cover, this.title});
