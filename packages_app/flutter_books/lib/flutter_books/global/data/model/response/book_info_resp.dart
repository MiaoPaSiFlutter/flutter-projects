...
class BookInfoResp {
  ...
  factory BookInfoResp(jsonStr) => jsonStr is String
      ? BookInfoResp.fromJson(json.decode(jsonStr))
      : BookInfoResp.fromJson(jsonStr);
  ...
  BookInfoResp.fromJson(jsonRes) {
    ...
    anchors = jsonRes['anchors'] ?? [];

    for (var anchorsItem in anchors!) {
      anchors!.add(anchorsItem);
    }

    gender = jsonRes['gender'] ?? [];

    for (var genderItem in gender!) {
      gender!.add(genderItem);
    }

    tags = jsonRes['tags'] ?? [];

    for (var tagsItem in tags!) {
      tags!.add(tagsItem);
    }

    rating = jsonRes['rating'] == null
        ? null
        : RatingBean.fromJson(jsonRes['rating']);
  }
