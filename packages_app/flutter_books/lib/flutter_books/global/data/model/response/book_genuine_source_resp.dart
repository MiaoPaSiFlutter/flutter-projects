...
class BookGenuineSourceResp {
  ...
  factory BookGenuineSourceResp(jsonStr) => jsonStr is String
      ? BookGenuineSourceResp.fromJson(json.decode(jsonStr))
      : BookGenuineSourceResp.fromJson(jsonStr);

  BookGenuineSourceResp.fromJson(jsonRes) {
    data = jsonRes['data'] ?? [];

    for (var dataItem in data!) {
      data!.add(BookGenuineSourceList.fromJson(dataItem));
    }
  }
...
class BookGenuineSourceList {
  ...
  BookGenuineSourceList.fromParams(
      {this.chaptersCount,
      this.isCharge,
      this.starting,
      this.id,
