...
def check_requirement(package):
    try:
        ...
        inquiry = input(
            "This script requires {0}. Do you want to install {0}? [y/n]".format(package))
...
class Runner:
    ...
    def main(self, argv):
        try:
            ops, args = getopt.getopt(
                argv, '-h-b:-u:-t:', ['help', 'build=', 'upload=', 'type='])
        ...
        for op, value in ops:
            if op in ('-h', '--help'):
                ...
                if ['all', 'debug', 'release'].__contains__(value) == False:
                    ...
                    self.buildMode = BuildMode.load(
                        ['all', 'debug', 'release'].index(value))
            elif op in ('-t', '--type'):
                count += 1
        ...
        self.p('{0}{1}{2}'.format(self.buildMode,
               self.buildType, self.uploadMode))
        # 开始编译和上传ipa
    ...
    def uploadFile(self, filePath, name='runner.ipa'):
        ...
        e = encoder.MultipartEncoder(
            fields={
                '_api_key': '57a3e5ff5ad3b6411d5a2680f760445a',
                #     'buildInstallType': '3',
                #     'buildPassword': '',
