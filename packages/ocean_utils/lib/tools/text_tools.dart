class OceanTextTools {
  ...
  ///字符串
  static String safeStr(String? data, {String placehold = ""}) {
    if (isEmpty(data)) {
      return placehold;
    }
    return data!;
  ...
  static String formatDigitPatternEnd(
    String text, {
    int digit = 4,
    String pattern = ' ',
  }) {
  ...
  static String formatDoubleComma3(
    Object num, {
    int digit = 3,
    String pattern = ',',
  }) {
    List<String> list = num.toString().split('.');
    String left = formatDigitPatternEnd(
      list[0],
      digit: digit,
      pattern: pattern,
    );
